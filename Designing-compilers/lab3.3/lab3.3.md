...
- Область видимости переменной начинается с оператора-объявления и заканчивается вместе с
  последовательностью операторов, где она определена.
        ...
        for param in self.params:
            if param.init is not None:
                ...
                if not is_assignable(param.type, init_type):
                    raise SemanticError(
                        param.name_coord,
                        f'Несовместимые типы присваивания: {type_to_str(param.type)} '
                        f':= {type_to_str(init_type)}',
                    )
            ...
            if declarator.init is not None:
                ...
                if not is_assignable(self.type, init_type):
                    raise SemanticError(
                        ...
                        f'Несовместимые типы присваивания: {type_to_str(self.type)} '
                        f':= {type_to_str(init_type)}',
                    )
...
class AssignStatement(Statement):
    ...
    def check(self, scope: Scope, context: SemanticContext) -> None:
        ...
        if not is_assignable(target_type, value_type):
            raise SemanticError(
                self.op_coord,
                f'Несовместимые типы присваивания: {type_to_str(target_type)} '
                f':= {type_to_str(value_type)}',
            )
    ...
    def check(self, scope: Scope, context: SemanticContext) -> None:
        ...
        if not is_assignable(loop_type, start_type):
            raise SemanticError(
                ...
                f'Несовместимые типы присваивания: {type_to_str(loop_type)} '
                f':= {type_to_str(start_type)}',
            )
...
class CallExpr(Expr):
    ...
    def check_call(self, scope: Scope, context: SemanticContext, require_value: bool) -> TypeNode | None:
        ...
        if len(self.args) != len(symbol.param_types):
            raise SemanticError(
                self.function_coord,
                f'Неверное количество аргументов функции {self.function}: '
                f'ожидалось {len(symbol.param_types)}, получено {len(self.args)}',
            )
        ...
        for i, (arg, expected_type) in enumerate(zip(self.args, symbol.param_types), start=1):
            ...
            if actual_type != expected_type:
                raise SemanticError(
                    arg.coord.start,
                    f'Неверный тип аргумента {i} функции {self.function}: '
                    f'ожидался {type_to_str(expected_type)}, '
                    f'получен {type_to_str(actual_type)}',
                )
...
class IndexExpr(Expr):
    ...
    def check(self, scope: Scope, context: SemanticContext) -> TypeNode:
        ...
        if not isinstance(array_type, ArrayType):
            raise SemanticError(
                self.op_coord,
                f'Недопустимый тип операнда: индексация {type_to_str(array_type)}',
            )
        if index_type not in (INT_TYPE, CHAR_TYPE):
            raise SemanticError(
                self.op_coord,
                f'Недопустимые типы операндов: {type_to_str(array_type)} '
                f'индекс {type_to_str(index_type)}',
            )
...
class UnaryOpExpr(Expr):
    ...
    def check(self, scope: Scope, context: SemanticContext) -> TypeNode:
        ...
        raise SemanticError(
            self.op_coord,
            f'Недопустимый тип операнда: {self.op} {type_to_str(expr_type)}',
        )
...
class BinaryOpExpr(Expr):
    ...
    def check(self, scope: Scope, context: SemanticContext) -> TypeNode:
        ...
        if result_type is None:
            raise SemanticError(
                self.op_coord,
                f'Недопустимые типы операндов: {type_to_str(left_type)} '
                f'{self.op} {type_to_str(right_type)}',
            )
        return result_type
    ...
    def infer_type(self, left_type: TypeNode, right_type: TypeNode) -> TypeNode | None:
        ...
        if self.op in ('=', '<>'):
            ...
            if left_type == BOOL_TYPE and right_type == BOOL_TYPE:
                return BOOL_TYPE
            if (
                isinstance(left_type, ArrayType)
                and isinstance(right_type, ArrayType)
...
def check_statement_sequence(
    statements: list[Statement], scope: Scope, context: SemanticContext
) -> None:
...
(
    ...
    kw,
    (
        'define int char bool array if elsif else then while do to step return '
        'assert new mod and or xor not NULL T F end'
    ).split(),
)
...
do
  x := x - 1
while x > 0;
assert args <> NULL and ok = T; *** комментарий
return 0
end
...
# Вывод
В ходе лабораторной работы были получены навыки реализации семантического анализа.
