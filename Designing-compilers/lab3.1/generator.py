        ...
        while stack:
            ...
            if self.is_terminal(top):
                if top == current.tag:
                    ...
                    continue
                self.errors.append(
                    f"{current.coord}: expected {self.symbol_name(top)}, got {self.symbol_name(current.tag)}"
                )
                return None
...
class GrammarScanner:
    ...
    def next_token(self) -> Token:
        while True:
            ...
            bad = ""
            while self.cur.cp is not None and not self.cur.is_whitespace() and self.cur.cp not in ',{}<>:@";':
                bad += self.cur.cp
                self.cur.next()
            if not bad:
                bad = self.cur.cp or ""
    ...
    def validate_declarations(self) -> Optional[str]:
        ...
        for decl in self.spec.declarations:
            if decl.name in seen:
                self.add_error(
                    decl.coord,
                    f"nonterminal {decl.name!r} is declared twice; first declaration at {seen[decl.name]}",
                )
            else:
        ...
        if len(starts) > 1:
            for decl in starts[1:]:
                self.add_error(decl.coord, f"more than one axiom is specified; extra axiom {decl.name!r}")
