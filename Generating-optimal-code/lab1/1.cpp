...
struct IRInstruction {
    ...
    IRInstruction(int i = -1, IROpcode op = IROpcode::ASSIGN) : id(i), opcode(op) {}
...
public:
    ...
    std::vector<CFGBlock*> getReachableBlocks() const {
        std::vector<CFGBlock*> result;
        for (const auto& block : blocks) {
            if (block->preOrder != 0) result.push_back(block.get());
        }
        return result;
    }
    ...
    void computeOrders() {
        ...
        for (const auto& block : blocks) {
            if (block->preOrder != 0) reachable.push_back(block.get());
        }
...
private:
    void computeDominators() {
        std::vector<CFGBlock*> reachable = cfg->getReachableBlocks();
        ...
        for (const auto& block : cfg->getAllBlocks()) {
            if (block->preOrder == 0) {
                doms[block.get()] = {block.get()};
                idom[block.get()] = nullptr;
            }
        }
    }
    ...
    void computeDominanceFrontier() {
        for (const auto& block : cfg->getAllBlocks()) {
            CFGBlock* b = block.get();
            if (b->predecessors.size() < 2) continue;

            CFGBlock* bIDom = getIDom(b);
            for (CFGBlock* pred : b->predecessors) {
                if (pred->preOrder == 0) continue;
                CFGBlock* runner = pred;
                while (runner && runner != bIDom) {
                    domFrontier[runner].insert(b);
...
public:
    ...
    int newInstrId() { return nextInstrId++; }
    ...
    IRValue emitBinary(IROpcode opcode, const IRValue& left, const IRValue& right) {
        ...
        IRInstruction instr(newInstrId(), opcode);
    ...
    void emitAssignment(const std::string& var, const IRValue& value) {
        IRInstruction instr(newInstrId(), IROpcode::ASSIGN);
    ...
    void emitReturn(const IRValue& value) {
        ...
        IRInstruction instr(newInstrId(), IROpcode::RET);
        instr.arg1 = value;
    ...
    void emitConditionalBranch(const IRValue& cond, CFGBlock* trueBlock, CFGBlock* falseBlock) {
        if (!currentBlock || currentBlock->hasTerminator()) return;
        IRInstruction instr(newInstrId(), IROpcode::COND_BR);
        instr.arg1 = cond;
    ...
    void emitBranch(CFGBlock* target) {
        if (!currentBlock || currentBlock->hasTerminator()) return;
        IRInstruction instr(newInstrId(), IROpcode::BR);
...
private:
    ...
    std::map<std::string, int> versionCounter;
    std::map<std::string, std::stack<int>> versionStack;
    int nextPhiId = 100000;
...
private:
    ...
    void placePhiFunctions() {
        for (auto& entry : varDefBlocks) {
            const std::string& varName = entry.first;
            std::set<CFGBlock*> defBlocks = entry.second;
            std::set<CFGBlock*> workList = defBlocks; 
            std::set<CFGBlock*> hasPhi;

            while (!workList.empty()) {
                CFGBlock* x = *workList.begin();
                workList.erase(workList.begin());

                for (CFGBlock* y : domInfo->getDomFrontier(x)) {  
                    if (y == cfg->getExit()) {
                        continue;
                    }
                    if (!hasPhi.count(y)) {
    ...
    void insertPhiInstructions() {
        for (const auto& blockPtr : cfg->getAllBlocks()) {
            ...
            for (const std::string& var : block->phiVariables) {
                IRInstruction phi(nextPhiId++, IROpcode::PHI);
    ...
    void renameVariables() {
        for (const auto& entry : varDefBlocks) {
            versionCounter[entry.first] = 0;
            versionStack[entry.first] = std::stack<int>();
        }

        renameBlock(cfg->getEntry());
    }

    void renameBlock(CFGBlock* block) {

        std::map<std::string, int> pushedCount;

        for (IRInstruction& instr : block->instructions) {
            if (instr.opcode != IROpcode::PHI) continue;

            std::string var = instr.result.name;
            int version = versionCounter[var]++;
            instr.result.version = version;
            versionStack[var].push(version);
            pushedCount[var]++;
        }
        ...
        for (IRInstruction& instr : block->instructions) {
            ...
            renameUse(instr.arg1);
            renameUse(instr.arg2);
        ...
        for (CFGBlock* succ : block->successors) {
            for (IRInstruction& instr : succ->instructions) {
                if (instr.opcode != IROpcode::PHI) continue;

                const std::string& var = instr.result.name;
                if (!versionStack[var].empty()) {
                    instr.phiArgs.push_back({block->name, IRValue(var, versionStack[var].top())});
                } else {
                    instr.phiArgs.push_back({block->name, IRValue(var)});
                }
            }
        }

        for (CFGBlock* child : domInfo->getChildren(block)) {
            renameBlock(child);
        }

        for (auto it = pushedCount.rbegin(); it != pushedCount.rend(); ++it) {
            const std::string& var = it->first;
            int count = it->second;
            while (count-- > 0 && !versionStack[var].empty()) {
                versionStack[var].pop();
            }
        }
    }

    void renameUse(IRValue& value) {
        if (!isUserVariableName(value.name)) return;
        if (!versionStack[value.name].empty()) {
            value.version = versionStack[value.name].top();
        }
    }
};
