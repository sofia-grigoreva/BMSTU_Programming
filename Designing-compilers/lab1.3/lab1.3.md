...
- Идентификаторы: последовательности буквенных символов Unicode и десятичных цифр,
  начинающиеся и заканчивающиеся на букву.
- Числовые литералы: последовательности шестнадцатеричных цифр (чтобы литерал не был
  похож на идентификатор, его можно предварять нулём, цифры в любом регистре).
- Ключевые слова: «qeq», «xx», «xxx».

## Лексический домен для защиты
Строки начинаются и заканчиваются на обратную кавычку, могут пересекать границы строк текста.
        ...
        while True:
            ...
            if self.is_letter(ch):
                ...
                while (
                    ...
                    and not self.is_letter(self.cur.cp)
                    and not self.is_hex_digit(self.cur.cp)
                ):
                    bad += self.cur.cp
                    self.cur.next()
...
```
IDENT (1, 1)-(1, 6): а222a
KEYWORD (1, 7)-(1, 10): qeq
IDENT (1, 11)-(1, 18): abcds7s
KEYWORD (2, 1)-(2, 3): xx
KEYWORD (2, 4)-(2, 7): xxx
IDENT (2, 8)-(2, 11): abc
NUMBER (3, 1)-(3, 8): 0xabcdef
NUMBER (3, 9)-(3, 12): 0xff
NUMBER (3, 13)-(3, 17): 0xabc
NUMBER (3, 18)-(3, 22): 0x1a2b
STRING (5, 1)-(6, 6):  bnmbmbmb
b mb
STRING (8, 1)-(8, 16): incomplete!!!!
END_OF_PROGRAM (8, 16)-(8, 16)
Error (8, 1): String must end with `
```

# Вывод
В ходе выполнения лабораторной работы были освоены базовые принципы построения
лексического анализатора в объектно-ориентированном стиле: выделение доменов токенов,
работа с позициями в исходном тексте и организация структуры сканера.